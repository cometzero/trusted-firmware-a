...
static inline bool apollo_qvp_is_valid_tfp_ierr(uint64_t tfp_source)
{
...
static void apollo_qvp_check_tfp_error(uint64_t err_status)
{
	...
	if ((err_status & ERX_STATUS_V) &&
		...
		if (!apollo_qvp_is_valid_tfp_ierr(ERX_STATUS_IERR(err_status))) {
...
static void apollo_qvp_setup_cpu_ras_config(void)
{
 ...
 */
void apollo_qvp_ras_init_per_cpu(void)
{
	apollo_qvp_setup_cpu_ras_config();
}
 ...
 *
 */
void apollo_qvp_css_pwr_domain_on_finish(const psci_power_state_t *target_state)
{
	css_pwr_domain_on_finish(target_state);
	apollo_qvp_ras_init_per_cpu();
}
 ...
 */
static void apollo_qvp_set_error_notification_irq(unsigned int irq)
{
...
static int apollo_qvp_ras_cpu_intr_handler(
	...
	const struct err_handler_data *const data)
{
	...
	size_t esb_len =
		cper_write_cpu_record((void *)APOLLO_QVP_CPER_BUF_BASE, APOLLO_QVP_CPER_BUF_SIZE);

	if (esb_len) {
		print_cper((const void *)APOLLO_QVP_CPER_BUF_BASE);
		flush_dcache_range(APOLLO_QVP_CPER_BUF_BASE, esb_len);
		VERBOSE("RAS: ESB written len=%zu @0x%lx\n",
				esb_len, (unsigned long)APOLLO_QVP_CPER_BUF_BASE);
	}

	apollo_qvp_check_tfp_error(read_erxstatus_el1());
	...
	plat_ic_end_of_interrupt(data->interrupt);
	apollo_qvp_set_error_notification_irq(ERROR_NOTIFICATION_IRQ);
	return 0;
}

/* RAS error record list definition, used by the common RAS framework. */
static struct err_record_info plat_err_records[] = {
	ERR_RECORD_SYSREG_V1(0, 1, NULL, &apollo_qvp_ras_cpu_intr_handler, 0),
};
