...
{							\
		.scmi_mbx_mem = APOLLO_QVP_SCMI_PFDI_MONITOR_BASE + \
			(APOLLO_QVP_SCMI_PFDI_MONITOR_SIZE_PER_CHANNEL * channel_id), \
...
#if USE_GIC_DRIVER == 3
static const uintptr_t apollo_qvp_gicr_base_addrs[] = {
	GICR_BASE_VIEW0_0_0,
	GICR_BASE_VIEW0_0_1,
	GICR_BASE_VIEW0_0_2,
	GICR_BASE_VIEW0_0_3,
...
#if PFDI_SUPPORT
static int apollo_qvp_pwr_domain_on(u_register_t mpidr)
{
	uint64_t ft_id;
...
{
#if PFDI_SUPPORT
	ops->pwr_domain_on = apollo_qvp_pwr_domain_on;
#endif
	ops->pwr_domain_on_finish = apollo_qvp_css_pwr_domain_on_finish;
...
#if USE_GIC_DRIVER == 3
	gic_set_gicr_frames(apollo_qvp_gicr_base_addrs);
#endif
	generic_delay_timer_init();
	apollo_qvp_ras_init_per_cpu();
}

void apollo_qvp_bl31_plat_runtime_setup(void)
{
...
{
	apollo_qvp_bl31_plat_runtime_setup();
}
