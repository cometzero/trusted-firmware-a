...
static bool apollo_qvp_is_measured_boot_skippable(void)
{
	int ret;
...
void bl2_plat_mboot_init(void)
{
	...
	apollo_qvp_skip_mboot = apollo_qvp_is_measured_boot_skippable();

	rse_measured_boot_init(apollo_qvp_rse_mboot_metadata);
}
...
int plat_mboot_measure_image(unsigned int image_id, image_info_t *image_data)
{
	...
	if (apollo_qvp_skip_mboot) {
		return 0;
	}

	/* Calculate image hash and record data in RSE */
	err = rse_mboot_measure_and_record(apollo_qvp_rse_mboot_metadata,
...
int plat_mboot_measure_key(const void *pk_oid, const void *pk_ptr,
			   size_t pk_len)
{
	return rse_mboot_set_signer_id(apollo_qvp_rse_mboot_metadata, pk_oid, pk_ptr,
				       pk_len);
}
