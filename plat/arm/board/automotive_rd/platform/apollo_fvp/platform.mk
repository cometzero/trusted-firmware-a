...
APOLLO_FVP_BASE		 =	plat/arm/board/automotive_rd/platform/apollo_fvp
APOLLO_FVP_CPU_SOURCES	:=	lib/cpus/aarch64/cortex_a720_ae.S

PLAT_INCLUDES		+=	-I${APOLLO_FVP_BASE}/include/ 	\
				-I${APOLLO_FVP_BASE}/ras/include/	\
				-Idrivers/arm/mhu
...
ifeq (${PFDI_SUPPORT}, 1)
BL31_SOURCES 		+=	${APOLLO_FVP_BASE}/cpu_self_test.c \
				${APOLLO_FVP_BASE}/apollo_fvp_sbist.c \
				${PFDI_SOURCE}
endif
...
PLAT_BL_COMMON_SOURCES	+=	${APOLLO_FVP_BASE}/apollo_fvp_plat.c	\
				${APOLLO_FVP_BASE}/include/apollo_fvp_helpers.S

BL2_SOURCES	+=	${APOLLO_FVP_CPU_SOURCES}	\
			${APOLLO_FVP_BASE}/apollo_fvp_err.c	\
			${APOLLO_FVP_BASE}/apollo_fvp_bl2_mem_params_desc.c	\
			lib/utils/mem_region.c	\
			drivers/arm/sbsa/sbsa.c	\
			plat/arm/common/arm_nor_psci_mem_protect.c

BL31_SOURCES	+=	${APOLLO_FVP_CPU_SOURCES}	\
			${APOLLO_FVP_BASE}/apollo_fvp_bl31_setup.c	\
			${APOLLO_FVP_BASE}/apollo_fvp_topology.c	\
			${APOLLO_FVP_BASE}/ras/apollo_fvp_ras.c	\
			${APOLLO_FVP_BASE}/ras/apollo_fvp_ras_helpers.S \
...
BL31_SOURCES 	+=	$(APOLLO_FVP_BASE)/apollo_fvp_cper.c
PLAT_INCLUDES	+=	-I$(APOLLO_FVP_BASE)/../common/include
endif
endif

ifeq (${TRUSTED_BOARD_BOOT}, 1)
BL2_SOURCES	+=	${APOLLO_FVP_BASE}/apollo_fvp_trusted_board_boot.c
FIP_BL2_ARGS	:=	tb-fw
$(eval $(call TOOL_ADD_PAYLOAD,${BUILD_PLAT}/tb_fw.crt,--tb-fw-cert))
endif

# Pass variant values to platform build. Keep accepting RD_ASPEN_VARIANT
# while Apollo FVP inherits the RD-Aspen machine configuration.
ifdef RD_ASPEN_VARIANT
APOLLO_FVP_VARIANT ?= $(RD_ASPEN_VARIANT)
endif
APOLLO_FVP_VARIANT ?= cfg1
APOLLO_FVP_VARIANTS := cfg1 cfg2 emu fpga

ifeq ($(filter $(APOLLO_FVP_VARIANT), $(APOLLO_FVP_VARIANTS)),)
$(error APOLLO_FVP_VARIANT must be one of: $(APOLLO_FVP_VARIANTS). Got '$(APOLLO_FVP_VARIANT)')
endif

$(eval $(call add_define,APOLLO_FVP_VARIANT_$(call uppercase,$(APOLLO_FVP_VARIANT))))
ifneq ($(filter $(APOLLO_FVP_VARIANT), cfg1 cfg2),)
$(eval $(call add_define,APOLLO_FVP_VARIANT_FVP))
endif
ifneq ($(filter $(APOLLO_FVP_VARIANT), emu fpga),)
$(eval $(call add_define,APOLLO_FVP_VARIANT_RTL))
endif

# Select per-variant HW config DTS before listing FDT_SOURCES.
ifneq ($(filter $(APOLLO_FVP_VARIANT),emu fpga),)
APOLLO_FVP_HW_CONFIG_DTS	:= fdts/apollo_fvp_rtl.dts
else ifneq ($(filter $(APOLLO_FVP_VARIANT),cfg1 cfg2),)
APOLLO_FVP_HW_CONFIG_DTS	:= fdts/apollo_fvp_fvp.dts
endif
...
FDT_SOURCES	+=	${APOLLO_FVP_BASE}/fdts/${PLAT}_fw_config.dts	\
			$(APOLLO_FVP_HW_CONFIG_DTS) \
			${APOLLO_FVP_BASE}/fdts/${PLAT}_optee_spmc_manifest.dts

FW_CONFIG	:=	${BUILD_PLAT}/fdts/${PLAT}_fw_config.dtb
HW_CONFIG	:=	${BUILD_PLAT}/fdts/$(basename $(notdir $(APOLLO_FVP_HW_CONFIG_DTS))).dtb
...
ifeq (${MEASURED_BOOT},1)
	...
	PLAT_BL_COMMON_SOURCES	+= ${APOLLO_FVP_BASE}/apollo_fvp_measured_boot.c
	BL2_SOURCES		+= drivers/arm/css/sds/sds.c
endif

ifeq ($(ENABLE_RSE_COMMS_BL2),1)
BL2_SOURCES	+=	${APOLLO_FVP_BASE}/apollo_fvp_rse_comms.c	\
			${RSE_COMMS_SOURCES}
endif
